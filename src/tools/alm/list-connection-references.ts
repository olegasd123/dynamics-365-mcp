...
const listConnectionReferencesSchema = {
  environment: z.string().optional().describe("Environment name"),
  nameFilter: z.string().optional().describe("Optional filter for display name or logical name"),
  solution: z.string().optional().describe("Optional solution display name or unique name"),
};

type ListConnectionReferencesParams = ToolParams<typeof listConnectionReferencesSchema>;

export async function handleListConnectionReferences(
  { environment, nameFilter, solution }: ListConnectionReferencesParams,
...
export const listConnectionReferencesTool = defineTool({
  name: "list_connection_references",
  description: "List connection references with connector and connection status details.",
  schema: listConnectionReferencesSchema,
  handler: handleListConnectionReferences,
});
...
export function registerListConnectionReferences(
  ...
  registerTool(server, listConnectionReferencesTool, { config, client });
}
