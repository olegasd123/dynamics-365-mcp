...
export async function handleComparePluginAssemblies(
  {
    sourceEnvironment,
    targetEnvironment,
    assemblyName,
  ...
  try {
    ...
    if (assemblyName && sourceItems.length > 0 && targetItems.length > 0 && imageResult) {
      ...
      text += formatDiffResult(imageResult, sourceEnvironment, targetEnvironment, "displayName");
    }
...
export const comparePluginAssembliesTool = defineTool({
  ...
  handler: handleComparePluginAssemblies,
});
