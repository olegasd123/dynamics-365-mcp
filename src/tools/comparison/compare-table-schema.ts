...
const compareTableSchemaSchema = {
...
type CompareTableSchemaParams = ToolParams<typeof compareTableSchemaSchema>;

export async function handleCompareTableSchema(
  { sourceEnvironment, targetEnvironment, table, targetTable }: CompareTableSchemaParams,
...
export const compareTableSchemaTool = defineTool({
  name: "compare_table_schema",
  description: "Compare one Dataverse table schema across two environments.",
  schema: compareTableSchemaSchema,
  handler: handleCompareTableSchema,
});
...
export function registerCompareTableSchema(
  ...
  registerTool(server, compareTableSchemaTool, { config, client });
}
