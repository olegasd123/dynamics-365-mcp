...
const getCustomApiDetailsSchema = {
  environment: z.string().optional().describe("Environment name"),
  apiName: z.string().describe("Custom API display name or unique name"),
};

type GetCustomApiDetailsParams = ToolParams<typeof getCustomApiDetailsSchema>;

export async function handleGetCustomApiDetails(
  { environment, apiName }: GetCustomApiDetailsParams,
...
export const getCustomApiDetailsTool = defineTool({
  name: "get_custom_api_details",
  description: "Show one Custom API with request and response metadata.",
  schema: getCustomApiDetailsSchema,
  handler: handleGetCustomApiDetails,
});
...
export function registerGetCustomApiDetails(
  ...
  registerTool(server, getCustomApiDetailsTool, { config, client });
}
