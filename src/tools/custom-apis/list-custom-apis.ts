import { z } from "zod";
import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type { AppConfig } from "../../config/types.js";
import { getEnvironment } from "../../config/environments.js";
import type { DynamicsClient } from "../../client/dynamics-client.js";
import { defineTool, registerTool, type ToolContext, type ToolParams } from "../tool-definition.js";
import { createToolErrorResponse, createToolSuccessResponse } from "../response.js";
import { formatTable } from "../../utils/formatters.js";
import { listCustomApis } from "./custom-api-metadata.js";

const listCustomApisSchema = {
  environment: z.string().optional().describe("Environment name"),
  nameFilter: z.string().optional().describe("Optional name or unique name filter"),
};

type ListCustomApisParams = ToolParams<typeof listCustomApisSchema>;

export async function handleListCustomApis(
  { environment, nameFilter }: ListCustomApisParams,
  { config, client }: ToolContext,
) {
  try {
    const env = getEnvironment(config, environment);
    const apis = await listCustomApis(env, client, nameFilter);

    if (apis.length === 0) {
      const text = `No custom APIs found in '${env.name}'.`;
      return createToolSuccessResponse("list_custom_apis", text, text, {
        environment: env.name,
        nameFilter: nameFilter || null,
        count: 0,
        items: [],
      });
    }

    const rows = apis.map((api) => [
      api.name,
      api.uniquename,
      api.bindingTypeLabel,
      api.boundentitylogicalname || "-",
      api.isfunction ? "Function" : "Action",
      api.allowedProcessingStepLabel,
      api.workflowsdkstepenabled ? "Yes" : "No",
      api.stateLabel,
    ]);

    const text = `## Custom APIs in '${env.name}'${nameFilter ? ` (filter='${nameFilter}')` : ""}\n\nFound ${apis.length} custom API(s).\n\n${formatTable(
      [
        "Name",
        "Unique Name",
        "Binding",
        "Bound Entity",
        "Kind",
        "Step Type",
        "Workflow Step",
        "State",
      ],
      rows,
    )}`;

    return createToolSuccessResponse(
      "list_custom_apis",
      text,
      `Found ${apis.length} custom API(s) in '${env.name}'.`,
      {
        environment: env.name,
        nameFilter: nameFilter || null,
        count: apis.length,
        items: apis,
      },
    );
  } catch (error) {
    return createToolErrorResponse("list_custom_apis", error);
  }
}

export const listCustomApisTool = defineTool({
  name: "list_custom_apis",
  description: "List Dataverse Custom APIs with binding and execution settings.",
  schema: listCustomApisSchema,
  handler: handleListCustomApis,
});

export function registerListCustomApis(
  server: McpServer,
  config: AppConfig,
  client: DynamicsClient,
) {
  registerTool(server, listCustomApisTool, { config, client });
}
