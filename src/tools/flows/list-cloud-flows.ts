...
type ListCloudFlowsParams = ToolParams<typeof listCloudFlowsSchema>;

export async function handleListCloudFlows(
  { environment, status, nameFilter, solution }: ListCloudFlowsParams,
...
export const listCloudFlowsTool = defineTool({
  name: "list_cloud_flows",
  description: "List cloud flows stored in Dataverse workflow metadata.",
  schema: listCloudFlowsSchema,
  handler: handleListCloudFlows,
});
...
export function registerListCloudFlows(
  ...
  registerTool(server, listCloudFlowsTool, { config, client });
}
