...
type GetFormDetailsParams = ToolParams<typeof getFormDetailsSchema>;

export async function handleGetFormDetails(
  { environment, formName, table, type, solution }: GetFormDetailsParams,
...
export const getFormDetailsTool = defineTool({
  name: "get_form_details",
  description: "Show one form with a normalized XML summary.",
  schema: getFormDetailsSchema,
  handler: handleGetFormDetails,
});
...
export function registerGetFormDetails(
  ...
  registerTool(server, getFormDetailsTool, { config, client });
}
