...
type ReleaseGateReportParams = ToolParams<typeof releaseGateReportSchema>;

export async function handleReleaseGateReport(
  { environment, solution, targetEnvironment, strict }: ReleaseGateReportParams,
    ...
    if (nextActions.length > 0) {
      ...
      lines.push(
        formatTable(
          ...
          nextActions.map((action) => [toTitleCase(action.priority), action.tool, action.reason]),
        ),
      );
    }
...
export const releaseGateReportTool = defineTool({
  name: "release_gate_report",
  description: "Build an opinionated go or no-go report for moving one solution.",
  schema: releaseGateReportSchema,
  handler: handleReleaseGateReport,
});
...
export function registerReleaseGateReport(
  ...
  registerTool(server, releaseGateReportTool, { config, client });
}
