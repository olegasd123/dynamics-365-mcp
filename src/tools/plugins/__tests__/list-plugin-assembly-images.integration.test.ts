...
describe("list_plugin_assembly_images tool", () => {
  ...
  it("supports the deprecated list_plugin_images alias with pluginName", async () => {
    const server = new FakeServer();
    const config = createTestConfig(["dev"]);
    ...
    expect(response.content[0].text).toContain("Plugin Assembly Images");
  });
});
