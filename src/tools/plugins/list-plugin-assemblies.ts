...
export async function handleListPluginAssemblies(
  {
    environment,
    filter,
    solution,
  ...
  try {
    ...
    const results = filterAssembliesByRegistration(inventory.assemblies, inventory.steps, filter);
...
export const listPluginAssembliesTool = defineTool({
  ...
  handler: handleListPluginAssemblies,
});
...
export function registerListPluginAssemblies(
  ...
  registerTool(server, listPluginAssembliesTool, { config, client });
}
