...
type ListTableColumnsParams = ToolParams<typeof listTableColumnsSchema>;

export async function handleListTableColumns(
  { environment, table, solution }: ListTableColumnsParams,
  ...
  try {
    ...
    const text = `## Columns: ${result.table.logicalName}\n\nEnvironment: ${env.name}\nSolution Filter: ${solution || "-"}\nFound ${result.columns.length} column(s).\n\n${formatTable(
      ["Logical Name", "Schema Name", "Type", "Required", "Primary", "Audit", "Search", "Details"],
      rows,
    )}`;
...
export const listTableColumnsTool = defineTool({
  name: "list_table_columns",
  description: "List Dataverse table columns with type, required level, and schema flags.",
  schema: listTableColumnsSchema,
  handler: handleListTableColumns,
});
...
export function registerListTableColumns(
  ...
  registerTool(server, listTableColumnsTool, { config, client });
}
