...
type AnalyzeUpdateTriggersParams = ToolParams<typeof analyzeUpdateTriggersSchema>;

export async function handleAnalyzeUpdateTriggers(
  { environment, table, changedAttributes }: AnalyzeUpdateTriggersParams,
...
export const analyzeUpdateTriggersTool = defineTool({
  name: "analyze_update_triggers",
  description: "Analyze what direct update triggers can run for a Dataverse table change.",
  schema: analyzeUpdateTriggersSchema,
  handler: handleAnalyzeUpdateTriggers,
});
...
export function registerAnalyzeUpdateTriggers(
  ...
  registerTool(server, analyzeUpdateTriggersTool, { config, client });
}
