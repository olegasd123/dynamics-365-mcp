...
const findTableUsageSchema = {
  environment: z.string().optional().describe("Environment name"),
  table: z.string().describe("Table logical name, schema name, or display name"),
};

type FindTableUsageParams = ToolParams<typeof findTableUsageSchema>;

export async function handleFindTableUsage(
  { environment, table }: FindTableUsageParams,
...
export const findTableUsageTool = defineTool({
  name: "find_table_usage",
  description: "Find where one Dataverse table is used across metadata assets.",
  schema: findTableUsageSchema,
  handler: handleFindTableUsage,
});
...
export function registerFindTableUsage(
  ...
  registerTool(server, findTableUsageTool, { config, client });
}
