...
import { listWebResourcesQuery, WEB_RESOURCE_TYPE } from "../../queries/web-resource-queries.js";
...
export function registerListWebResources(server: McpServer, config: AppConfig, client: DynamicsClient) {
  server.tool(
    "list_web_resources",
    "List web resources in Dynamics 365, optionally filtered by type or name.",
    {
      environment: z.string().optional().describe("Environment name"),
      type: z.enum(["html", "css", "js", "xml", "png", "jpg", "gif", "xap", "xsl", "ico", "svg", "resx"]).optional().describe("Filter by web resource type"),
    ...
    async ({ environment, type, nameFilter }) => {
      try {
        ...
        if (resources.length === 0) {
          return {
            content: [{ type: "text" as const, text: `No web resources found in '${env.name}' with the specified filters.` }],
          };
        }
