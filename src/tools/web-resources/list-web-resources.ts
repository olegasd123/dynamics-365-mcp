...
type ListWebResourcesParams = ToolParams<typeof listWebResourcesSchema>;

export async function handleListWebResources(
  { environment, type, nameFilter, solution, limit, cursor }: ListWebResourcesParams,
...
export const listWebResourcesTool = defineTool({
  name: "list_web_resources",
  description: "List web resources in Dynamics 365, optionally filtered by type or name.",
  schema: listWebResourcesSchema,
  handler: handleListWebResources,
});
...
export function registerListWebResources(
  ...
  registerTool(server, listWebResourcesTool, { config, client });
}
